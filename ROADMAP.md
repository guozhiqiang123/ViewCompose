...
3. instrumentation 已覆盖关键 smoke 回归路径，延迟 session 容器专项已覆盖 `LazyVerticalGrid`、`HorizontalPager` 与 `ModalBottomSheet`，`VerticalPager` 仍待补齐
...
| --- | --- | --- | --- |
| A：Overlay 稳定性收口 | In Progress | C:✅ U:✅ D:✅ UI:✅ | Overlay host 已统一 reconcile 模板，Dialog/Popup/ModalBottomSheet/反馈流均已回归 |
