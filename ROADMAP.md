...
14. 约束布局能力已落地：`viewcompose-widget-constraintlayout` + renderer `DeclarativeConstraintLayout`，支持 anchors/dimension/bias/baseline/guideline/barrier/chain/decoupled `ConstraintSet`
...
| ConstraintLayout | 已新增 `viewcompose-widget-constraintlayout` 与 renderer 映射，demo/preview 可验证核心约束能力 | 下一步补 MotionLayout interop 专题（保持 host-android 边界） |
