...
本文档基于对主题系统全部源码的逐行审计，识别出 **5 个设计维度** 的当前状态。

其中 2 个问题已在 THEMING.md 中描述过方向（但缺少代码级实证和执行路径），**2 个是全新发现**。
...
| **4** | Android 桥接补全 → typography + uiMode | 无对应（新发现） | 小 | 宿主集成开箱即用 |
| **5** | 引入更细 local（LocalTextStyle, ProvideContentColor） | Phase C | 小 | 减少 UiThemeOverride 职责膨胀 |
| **6** | Eager resolution 不动 | — | 0 | 基准已验证 2.3ms，无需优化 |
...
| 文档 | 本文档的关系 |
|------|-------------|
| [THEMING.md](THEMING.md) | THEMING.md 定义方向，本文补充代码级实证和 2 个新发现 |
...
| 4 | 在使用 Material 3 主题的宿主 app 中，`UiTheme(androidContext = this)` 自动获取正确的字体大小和 light/dark 模式 |
| 5 | 在不使用 `UiThemeOverride` 的情况下，可以通过 `ProvideContentColor` 改变子树内所有 Text 的默认颜色 |
